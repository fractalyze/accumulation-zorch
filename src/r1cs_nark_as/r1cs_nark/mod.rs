    ...
    S: CryptographicSponge<ConstraintF<G>>,
{
    ...
    /// Proves that some R1CS relation holds. The `B = CpuBackend` specialization
    /// of [`prove_with_backend`](Self::prove_with_backend).
    pub fn prove<C: ConstraintSynthesizer<G::ScalarField>>(
        ...
        Self::prove_with_backend::<C, CpuBackend>(ipk, r1cs, make_zk, sponge, rng)
    }

    /// Proves that some R1CS relation holds, routing each Pedersen commitment
    /// (MSM) through the [`MsmBackend`](crate::backend::MsmBackend) `B`.
    pub fn prove_with_backend<C: ConstraintSynthesizer<G::ScalarField>, B: MsmBackend<G>>(
        ipk: &IndexProverKey<G>,
        r1cs: C,
        make_zk: bool,
        sponge: Option<S>,
        ...
        let comm_a = B::commit(&ipk.ck, &z_a, a_blinder);
        let comm_b = B::commit(&ipk.ck, &z_b, b_blinder);
        let comm_c = B::commit(&ipk.ck, &z_c, c_blinder);
        ...
        let first_round_randomness = if make_zk {
            ...
            let comm_r_a = B::commit(&ipk.ck, r_a.as_ref().unwrap(), r_a_blinder);
            let comm_r_b = B::commit(&ipk.ck, r_b.as_ref().unwrap(), r_b_blinder);
            let comm_r_c = B::commit(&ipk.ck, r_c.as_ref().unwrap(), r_c_blinder);
            end_timer!(commit_time);
            ...
            let comm_1 = B::commit(&ipk.ck, &cross_product, blinder_1);
            end_timer!(commit_time);
            ...
            let comm_2 = B::commit(&ipk.ck, &r_a_r_b_product, blinder_2);
            end_timer!(commit_time);
