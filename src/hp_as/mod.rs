use crate::backend::{CpuBackend, MsmBackend};
    ...
    S: CryptographicSponge<ConstraintF<G>>,
{
    ...
    fn generate_prover_randomness<B: MsmBackend<G>>(
        ...
        let comm_1 = B::commit(prover_key, a.as_slice(), Some(rand_1));
        let comm_2 = B::commit(prover_key, b.as_slice(), Some(rand_2));
        ...
        let comm_3 = {
            ...
            B::commit(prover_key, rand_prods_sum.as_slice(), Some(rand_3))
        };
    ...
    fn compute_product_poly_comm<B: MsmBackend<G>>(
        ...
        for (i, t_vec) in t_vecs.iter().enumerate() {
            ...
            let comm = B::commit(ck, t_vec.as_slice(), None);
    ...
    S: CryptographicSponge<ConstraintF<G>>,
{
    ...
    where
        ...
        S: 'a,
    {
        Self::prove_with_backend::<CpuBackend>(
            prover_key,
            inputs,
            old_accumulators,
            make_zk,
...
where
    ...
    S: CryptographicSponge<ConstraintF<G>>,
{
    /// [`AccumulationScheme::prove`] with every prover Pedersen commitment
    /// (MSM) routed through the [`MsmBackend`](crate::backend::MsmBackend) `B`.
    /// The trait `prove` is the `B = CpuBackend` specialization of this.
    pub fn prove_with_backend<'a, B: MsmBackend<G>>(
        prover_key: &PedersenCommitmentCK<G>,
        inputs: impl IntoIterator<Item = InputRef<'a, ConstraintF<G>, S, Self>>,
        old_accumulators: impl IntoIterator<Item = AccumulatorRef<'a, ConstraintF<G>, S, Self>>,
        make_zk: MakeZK<'_>,
        sponge: Option<S>,
    ) -> Result<(Accumulator<ConstraintF<G>, S, Self>, Proof<G>), BoxedError>
    where
        Self: 'a,
        S: 'a,
    {
        ...
        let (hiding_vecs, hiding_rands, hiding_comms) = if make_zk_enabled {
            ...
            let (hiding_vecs, hiding_rands, hiding_comms) = Self::generate_prover_randomness::<B>(
                prover_key,
                hp_vec_len,
                &all_input_witnesses,
                rng.unwrap(),
        ...
        let product_poly_comm = Self::compute_product_poly_comm::<B>(prover_key, &t_vecs)?;
        let proof = Proof {
            product_poly_comm,
            hiding_comms,
        };
