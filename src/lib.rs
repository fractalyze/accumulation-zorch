//! A faithful copy of the arkworks `ark-accumulation` native prove path
//! (`r1cs_nark_as` + `hp_as`) over the Pasta cycle, with every prover Pedersen
//! commitment (MSM) routed behind an [`MsmBackend`](crate::backend::MsmBackend)
//! so the inner multi-scalar multiplication can be swapped for a GPU backend.
//!
...
// The pure-CPU prover (default build) contains no unsafe code. The GPU backend
// (feature `gpu`) needs unsafe FFI into the zkx-pjrt plugin, so under that
// feature the crate-wide ban is downgraded to `deny` and the `gpu` module alone
// is granted a scoped exception.
...
/// The pluggable multi-scalar-multiplication seam (`MsmBackend` + `CpuBackend`).
pub mod backend;
...
/// each curve's PJRT buffer-type tags. (The per-MSM `GpuBackend` retired in #331.)
#[cfg(feature = "gpu")]
#[allow(unsafe_code)]
pub mod gpu;

/// Thin consumer of the fused jax-exported prove core (feature `gpu`, zorch#313
/// Phase 3): loads `artifacts/prove_zk_general.mlirbc` (the general prover,
/// zorch#330 — assignment + randomness are runtime inputs) and runs the whole
/// prove as one PJRT call, replacing `GpuBackend`'s per-MSM dispatch.
#[cfg(feature = "gpu")]
#[allow(unsafe_code)]
pub mod fused;

/// Byte-identical oracle test vs the unmodified arkworks prover (test-only).
#[cfg(test)]
mod oracle;
