...
def sample_randomness(
    cv: Curve, rng: np.random.Generator, num_witness: int
) -> Randomness:
...
class FoldedAccumulator(NamedTuple):
    ...
    def to_decide(self) -> "Accumulator":
        ...
        return Accumulator(
            r1cs_input=self.r1cs_input,
            blinded_witness=self.blinded_witness,
            hp_a_vec=self.hp_a_vec,
            hp_b_vec=self.hp_b_vec,
            sigmas=self.sigma_abc,
            hp_rand=self.hp_rand,
        )
...
def _materialize_zk(
    cv: Curve, core_out: tuple, r1cs_r_input: list[int]
    ...
    acc = FoldedAccumulator(
        ...
        blinded_witness=np.asarray(cbw, dtype=cv.fr),
        sigma_abc=_fr_triple(csig),
...
def _serialize_acc_instance(
    cv: Curve,
    r1cs_input: FrVec,
    comm_a: np.ndarray,
    comm_b: np.ndarray,
    comm_c: np.ndarray,
    hp_instance: hp_as.Instance,
) -> bytes:
    ...
    out += (
        curve.point_to_bytes(cv, comm_a)
        + curve.point_to_bytes(cv, comm_b)
...
def _serialize_acc_witness(
    cv: Curve, blinded_witness: FrVec, hp_a_vec: frx.Array, hp_b_vec: frx.Array
) -> bytes:
    ...
    out += (
        curve.serialize_fr_vec(cv, hp_a_vec)
        + curve.serialize_fr_vec(cv, hp_b_vec)
        + b"\x00"
    )  # hp randomness None
...
def _build_no_zk_core(
    ...
    n = len(r1cs_input) + len(
        blinded_witness
    )  # z length = the circuit's num_vars (static)
    bases = curve.stack_affine(cv, generators[:rows])
    a_dense, b_dense, c_dense = (
        fnp.asarray(nark.to_dense(cv, m, n)) for m in (a, b, c)
    )
    ...
    @frx.jit
    def _core(
        bases: frx.Array, r1cs_input: frx.Array, blinded_witness: frx.Array
    ) -> tuple:
        ...
        hp = hp_as.prove_no_zk_core(
            cv,
            fnp.stack([comm_a, comm_b, comm_c]),
            a_arr,
            b_arr,
            supported_num_elems,
            params,
...
def prove_no_zk(
    ...
    acc_instance = _serialize_acc_instance(
        cv,
        r1cs_input,
        np.asarray(comm_a),
        np.asarray(comm_b),
...
def _serialize_proof_zk(
    cv: Curve,
    low: list[np.ndarray],
    high: list[np.ndarray],
    hiding_comms: hp_as.Instance,
    r1cs_r_input: list[int],
    comm_r: tuple[np.ndarray, np.ndarray, np.ndarray],
) -> bytes:
...
def _beta_challenges_frx(
    cv: Curve,
    params: Any,
    as_matrices_hash: bytes,
    input_inst_fe: frx.Array,
    proof_rand_fe: frx.Array,
    acc_inst_fe: frx.Array | None = None,
...
def _build_zk_core(
    ...
    **example** runtime arrays for lowering (the lowered core is seed-independent);
    ...
    ex_hp_rand = fnp.asarray(
        np.array([hp_hiding_a, hp_hiding_b, *hp_rand], dtype=cv.fr)
    )
    ...
    def _core(
        ...
        wit_arr: frx.Array,
        r_arr: frx.Array,
        blinders_arr: frx.Array,
        r_in_arr: frx.Array,
        r_wit_arr: frx.Array,
        as_rand_arr: frx.Array,
        hp_rand_arr: frx.Array,
        in_u8b: frx.Array,
        r_in_u8b: frx.Array,
    ) -> tuple:
        fr_one = fnp.asarray(np.array([1], dtype=cv.fr))
        ...
        comm_prod = lax.msm(
            fnp.concatenate([fr_one, gamma, gamma * gamma]),
        ...
        hp_core = hp_as.prove_zk_core(
            ...
            bases_h,
            id_pt,
            fnp.stack([blinded_comm_a, blinded_comm_b, comm_prod]),
            _mz(a_dense, zw),
            _mz(b_dense, zw),
            fnp.stack([nk.sigma_abc[0], nk.sigma_abc[1], nk.sigma_o]),
            supported_num_elems,
            params,
            hp_hiding_a,
            hp_hiding_b,
        ...
        beta = _beta_challenges_frx(
            cv, params, as_matrices_hash, inst_fe, pr_fe
        )  # (2,) = [1, c]
...
class Accumulator(NamedTuple):
    ...
    )  # witness.hp_witness.randomness.{rand_1,rand_2,rand_3}
...
def decide(
    ...
    c: nark.Matrix,
    generators: list[np.ndarray],
    hiding: np.ndarray | None,
    acc: Accumulator,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """`ASForR1CSNark::decide` recomputed (CPU group-reduction oracle, the fused
    GPU core's target — the byte-match counterpart of
    :func:`ipa_pc_as.decide_final_key`).
    ...
    sigma_a, sigma_b, sigma_c = (
        acc.sigmas if acc.sigmas is not None else (None, None, None)
    )
    ...
    rand_1, rand_2, rand_3 = (
        acc.hp_rand if acc.hp_rand is not None else (None, None, None)
    )
    hp_comm_1 = curve.pedersen_commit(
        cv, generators[: len(acc.hp_a_vec)], acc.hp_a_vec, hiding, rand_1
    )
    hp_comm_2 = curve.pedersen_commit(
        cv, generators[: len(acc.hp_b_vec)], acc.hp_b_vec, hiding, rand_2
    )
    hp_comm_3 = curve.pedersen_commit(
        cv, generators[: len(product)], product, hiding, rand_3
    )
...
def _build_zk_fold_core(
    ...
    hp_hiding_b: int,
    hp_rand: tuple[int, int, int],
    acc_r1cs_input: FrVec,
    acc_comms: list[np.ndarray],
    acc_blinded_witness: FrVec,
    acc_sigma_abc: tuple[int, int, int],
    acc_hp_a_vec: FrVec,
    acc_hp_b_vec: FrVec,
    acc_hp_rand: tuple[int, int, int],
) -> tuple:
    ...
    # comm_r = commit(M·(r1cs_r_input ‖ r1cs_r_witness)); the AS proof-randomness
    # reduce.
    ...
    def _core(bases_h: frx.Array, id_pt: frx.Array, acc_comms: frx.Array) -> tuple:
        ...
        combined_input = field.combine_vectors(
            fnp.asarray(
                np.array([acc_r1cs_input, r1cs_input, r1cs_r_input], dtype=cv.fr)
            ),
            beta,
        )
        ...
        combined_blinded_witness = field.combine_vectors(
            fnp.stack(
                [
                    fnp.asarray(np.array(acc_blinded_witness, dtype=cv.fr)),
                    nk.blinded_witness,
                    fnp.asarray(np.array(r1cs_r_witness, dtype=cv.fr)),
                ]
            ),
            beta,
        )
        combined_sigmas = (
            fnp.asarray(np.array(list(acc_sigma_abc), dtype=cv.fr)) * beta[0]
            + nk.sigma_abc * beta[1]
...
def _matrices_hashes(
    cv: Curve, a: nark.Matrix, b: nark.Matrix, c: nark.Matrix
) -> tuple[bytes, bytes]:
    ...
    return (
        nark.hash_matrices(cv, nark.PROTOCOL_NAME, a, b, c),
