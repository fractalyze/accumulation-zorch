...
def test_succinct_check_round_challenges_match_arkworks() -> None:
    for cv, ipa_fixture, sponge_fixture in _CURVES:
        ...
        assert len(got) == len(d["round_challenges"]), f"[{cv.name}] challenge count"
...
def test_check_poly_coeffs_and_eval_match_arkworks() -> None:
    ...
    for cv, ipa_fixture, sponge_fixture in _CURVES:
        ...
        assert len(coeffs) == len(d["coeffs"]), f"[{cv.name}] coeff count"
        for i, want_hex in enumerate(d["coeffs"]):
            got_hex = cv.fr(coeffs[i]).tobytes().hex()
            assert got_hex == want_hex, f"[{cv.name}] h(X) coeff[{i}]: {got_hex} != {want_hex}"

        got_eval = cv.fr(ipa_pc.evaluate(cv, challenges, point)).tobytes().hex()
        assert got_eval == d["eval_at_point"], (
...
def test_succinct_check_end_to_end_matches_arkworks() -> None:
...
def main() -> None:
    print("slice-1 IPA-PC succinct_check byte-match (Pallas + Vesta):")
    test_succinct_check_round_challenges_match_arkworks()
    test_check_poly_coeffs_and_eval_match_arkworks()
    test_succinct_check_end_to_end_matches_arkworks()
    print("ALL SLICE-1 IPA-PC CHECKS PASSED")
