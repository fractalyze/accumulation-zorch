...
def test_sponge_schedules_match_arkworks() -> None:
    for cv, fixture in _CURVES:
        data = json.loads(fixture.read_text())
        ark_le = b"".join(bytes.fromhex(h) for h in data["ark_le_hex"])
        assert len(data["ark_le_hex"]) == 117, "expected 117 ARK constants"
        ...
        for sched in data["schedules"]:
            ...
            for op in sched["ops"]:
                if op["op"] == "absorb":
                    ...
                    got = np.asarray(out)
                    assert got.shape[0] == op["n"], f"[{cv.name}] {sched['name']}: squeeze count"
...
def test_nonnative_truncated_squeeze_matches_arkworks() -> None:
    for cv, fixture in _CURVES:
        data = json.loads(fixture.read_text())
        ark_le = b"".join(bytes.fromhex(h) for h in data["ark_le_hex"])
        params = sponge.poseidon_params(cv, ark_le)
        ...
        for case in data["nonnative_squeeze"]:
            ...
            assert len(challenges) == len(case["challenges"])
...
def main() -> None:
    print("slice-2 Poseidon FS sponge byte-match (Pallas + Vesta):")
    test_sponge_schedules_match_arkworks()
    test_nonnative_truncated_squeeze_matches_arkworks()
    print("ALL SLICE-2 SPONGE CHECKS PASSED")
