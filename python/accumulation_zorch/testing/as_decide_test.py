...
_CURVES = [
    ...
    (
        curve.VESTA,
        _TESTDATA / "as_vesta_fixtures.json",
        _TESTDATA / "as_zk_vesta_fixtures.json",
    ),
]
...
def _take_fr_vec(buf: bytes, off: int) -> tuple[list[int], int]:
    ...
    off += 8
    vals = [
        int.from_bytes(buf[off + i * _FR_BYTES : off + (i + 1) * _FR_BYTES], "little")
        for i in range(n)
    ]
...
def _parse_witness(buf: bytes, zk: bool) -> tuple[
    list[int],
    list[int],
    list[int],
    ...
    def _take_opt_triple(off: int) -> tuple[tuple[int, int, int] | None, int]:
        ...
        triple = tuple(
            int.from_bytes(
                buf[off + i * _FR_BYTES : off + (i + 1) * _FR_BYTES], "little"
            )
            for i in range(3)
        )
    ...
    if zk:
        assert (
            hp_rand is not None and sigmas is not None
        ), "zk witness must carry Some randomness"
    else:
        assert (
            hp_rand is None and sigmas is None
        ), "no-zk witness must carry None randomness"
...
class AsDecideTest(absltest.TestCase):
    def _check(self, cv: curve.Curve, fixture: Path, zk: bool) -> None:
        ...
        for seed_entry in d["seeds"]:
            self.assertTrue(
                seed_entry["decide"],
                f"[{cv.name}, {mode}] fixture seed {seed_entry['seed']} not "
                f"arkworks-decided",
            )
            r1cs_input, want_comms = _parse_instance(
                bytes.fromhex(seed_entry["acc_instance_hex"])
            )
            ...
            acc = r1cs_nark_as.Accumulator(
                r1cs_input=r1cs_input,
                blinded_witness=blinded_witness,
                hp_a_vec=hp_a_vec,
                hp_b_vec=hp_b_vec,
                sigmas=sigmas,
                hp_rand=hp_rand,
            ...
            for label, g, w in zip(labels, got_hex, want_comms):
                self.assertEqual(
                    g,
                    w,
                    f"[{cv.name}, {mode}] seed {seed_entry['seed']} {label}: {g} != "
                    f"{w}",
                )
            print(
                f"  [{cv.name}, {mode}] seed {seed_entry['seed']}: decider recomputes "
    ...
    def test_mutation_breaks_match(self) -> None:
        ...
        self.assertNotEqual(
            got_comm_a, want_comms[0], "a perturbed blinded witness must change comm_a"
        )
        print(
            "  mutation check: a perturbed blinded witness diverges from the golden "
            "comm_a"
        )
