...
class HpZkTest(absltest.TestCase):
    def test_hp_zk_prove_matches_arkworks(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                ...
                f"{d['acc_instance_hex']}"
            ),
        )
        print(
            f"  zk HP accumulator instance byte-matches arkworks ({len(acc_inst)} "
            f"bytes)"
        )
        ...
        print(
            f"  zk HP accumulator witness byte-matches arkworks ({len(acc_wit)} bytes)"
        )
        ...
        print(
            f"  zk HP proof byte-matches arkworks ({len(proof)} bytes, low={len(low)} "
            f"high={len(high)})"
        )
    ...
    def test_mutation_breaks_match(self) -> None:
        ...
        self.assertNotEqual(
            proof.hex(),
            json.loads(_HP_ZK.read_text())["proof_hex"],
            "mutation did not change the proof",
        )
        print(
            "  mutation check: a perturbed hiding randomness diverges from the golden"
        )
