...
**The directory must be named explicitly, via `$ACCUMULATION_ZORCH_ARTIFACTS`.** For
these
three tests there is deliberately no `<repo>/artifacts` default: a default would have
to be
reached by resolving `__file__` back out of the runfiles tree into the source checkout,
so
the test would read a file Bazel has never heard of — and, not being a declared input,
one
...
`<manifest>/artifacts`. Three resolutions for one variable is one too many; draining
them
...
**A missing fixture is an error, not a skip.** These targets are `manual`, so the only
way
one runs is a deliberate request — and then a skip is never what the caller asked for.
It
would also be invisible: a skipped absltest case exits 0, which Bazel reports as
`PASSED`,
...
def fixture(name: str, dump: str) -> Path:
    ...
    `dump` is the cargo command that generates `name`; it is quoted back in the error,
    so
