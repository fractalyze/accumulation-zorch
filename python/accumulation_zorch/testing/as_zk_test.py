...
def _prove(
    d: Any, a: Any, b: Any, c: Any, generators: Any, s: Any
...
class AsZkTest(absltest.TestCase):
    ...
    def test_mutation_breaks_match(self) -> None:
        ...
        self.assertNotEqual(
            full,
            g["acc_instance_hex"] + g["acc_witness_hex"] + g["proof_hex"],
            "mutation no-op",
        )
        print(
            "  mutation check: a perturbed NARK blinder diverges from the golden bytes"
        )
