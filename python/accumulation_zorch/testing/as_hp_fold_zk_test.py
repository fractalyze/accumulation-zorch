...
def _hp_fold(d: Any, s: Any, params: Any) -> tuple:
    ...
    old_rand = fnp.asarray(
        np.array(
            [_fr(accw["hp_rand_1"]), _fr(accw["hp_rand_2"]), _fr(accw["hp_rand_3"])],
            dtype=cv.fr,
        )
    )
    ...
    def core(
        ...
        zw = fnp.concatenate(
            [fnp.asarray(np.array(input2, dtype=cv.fr)), nk.blinded_witness]
        )
    ...
    def test_hp_fold_matches_arkworks(self) -> None:
        ...
        for s in d["seeds"]:
            ...
            want_r = [
                ...
                for k in ("hp_rand_1", "hp_rand_2", "hp_rand_3")
            ]
            self.assertEqual(
                got_r,
                want_r,
            ...
            print(
                f"  [seed {s['seed']}] combined HP instance + witness byte-matches "
                f"arkworks "
