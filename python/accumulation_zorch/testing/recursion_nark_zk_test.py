...
Three Slice-3 pieces converge here vs the no-zk half-step:
  * the **on-device sparse M·z** (step: the six z_M / r_M reduces run as a
    scatter-free CSR prefix sum, not densified — the recursion R1CS densified is ~15
    GB),
...
_SPONGE = (
    Path(__file__).resolve().parents[2] / "testdata" / "sponge_vesta_fixtures.json"
)
...
class RecursionNarkZkTest(absltest.TestCase):
    ...
    def test_recursion_nark_zk_proof_matches_arkworks(self) -> None:
        ...
        print(
            ...
            f"({d['num_constraints']} constraints, {d['num_vars']} vars, {len(proof)} "
            f"bytes)"
        )
