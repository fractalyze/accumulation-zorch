...
_CURVES = [
    (
        curve.PALLAS,
        _TESTDATA / "ipa_as_fixtures.json",
        _TESTDATA / "sponge_fixtures.json",
    ),
    (
        curve.VESTA,
        _TESTDATA / "ipa_as_vesta_fixtures.json",
...
class IpaAsTest(absltest.TestCase):
    def test_as_prove_no_zk_accumulator_instance_matches_arkworks(self) -> None:
        for cv, as_fixture, sponge_fixture in _CURVES:
            ...
            self.assertEqual(
                got_comm,
                want_comm,
                f"[{cv.name}] combined commitment: {got_comm} != {want_comm}",
            )
            ...
            print(
                f"  [{cv.name}] AS no-zk accumulator instance (commitment, point, "
                f"evaluation) "
    ...
    def test_as_prove_no_zk_full_accumulator_matches_arkworks(self) -> None:
        ...
        for cv, as_fixture, sponge_fixture in _CURVES:
            ...
            print(
                f"  [{cv.name}] full AS no-zk accumulator (instance + IPA proof: "
    ...
    def test_decide_no_zk_size_d_msm_matches_final_comm_key(self) -> None:
        ...
        for cv, as_fixture, sponge_fixture in _CURVES:
            ...
            self.assertEqual(
                got,
                want,
                f"[{cv.name}] decider size-d MSM != final_comm_key: {got} != {want}",
            )
            print(
                f"  [{cv.name}] decider size-d MSM (= MSM(generators, h(X) coeffs)) "
