...
_CHALLENGE_BITS = min(
    CHALLENGE_SIZE, sponge.FR_CAPACITY
)  # squeeze window per challenge
...
def serialize_proof(cv: Curve, low: list[np.ndarray], high: list[np.ndarray]) -> bytes:
    ...
    out = struct.pack("<Q", len(low)) + b"".join(
        curve.point_to_bytes(cv, p) for p in low
    )
    out += struct.pack("<Q", len(high)) + b"".join(
        curve.point_to_bytes(cv, p) for p in high
    )
    return out + b"\x00"
...
def prove_no_zk_core(
    ...
    params: Any,
    base_sponge: DuplexSponge | None = None,
) -> HpNoZkCore:
...
def materialize_no_zk(
    core: HpNoZkCore,
) -> tuple[Instance, tuple[frx.Array, frx.Array], list[np.ndarray], list[np.ndarray]]:
...
def squeeze_mu_frx(
    cv: Curve, sp: DuplexSponge, num_inputs: int
) -> tuple[DuplexSponge, frx.Array]:
    ...
    if num_inputs > 1:
        sp, rest = sponge.squeeze_challenges_frx(
            sp, num_inputs - 1, _CHALLENGE_BITS, cv
        )
...
def _product_poly_comm_frx(
    bases: frx.Array, t_vecs: frx.Array, num_inputs: int
) -> tuple[list[frx.Array], list[frx.Array]]:
...
def _combine_randomness(
    cv: Curve, rands: list[int | None], challenges: list[int], hiding: int | None = None
) -> FrScalar:
...
def _prove_zk_segment(
    ...
    a_real: frx.Array,
    b_real: frx.Array,
    input_rand: frx.Array,
    old_inst: frx.Array | None = None,
    old_a: frx.Array | None = None,
    old_b: frx.Array | None = None,
    old_rand: frx.Array | None = None,
    hp_rand: frx.Array | None = None,
) -> HpZkCore:
    ...
    row1_comms = (
        fnp.concatenate([id_pt, id_pt, id_pt]) if old_inst is None else old_inst
    )
    ...
    t_vecs = field.t_vecs_zk(
        a,
        b,
        mu[:num_inputs],
        hiding_a_vec,
        hiding_b_vec,
        mu[num_inputs].reshape(1),
        mu[1].reshape(1),
    )
    ...
    cc1 = lax.msm(
        fnp.concatenate([combined, mu_n]), fnp.concatenate([comm_1s, hiding_comms[0:1]])
    )
    cc2 = lax.msm(
        fnp.concatenate([nu[:num_inputs], mu[1:2]]),
        fnp.concatenate([fnp.flip(comm_2s, 0), hiding_comms[1:2]]),
    )
    comm3_combine = lax.msm(
        fnp.concatenate([mu[:num_inputs], mu_n]),
    ...
    cc3 = lax.msm(
        fnp.concatenate([nu[num_inputs - 1 : num_inputs], fr_one, fr_one]),
    ...
    b_open = (
        field.combine_vectors(fnp.flip(b, 0), nu[:num_inputs]) + mu[1] * hiding_b_vec
    )
    a_rand = (
        input_rand[0] * combined[0]
        + row1_rand[0] * combined[1]
        + hr[0] * mu[num_inputs]
    )
    b_rand = row1_rand[1] * nu[0] + input_rand[1] * nu[1] + hr[1] * mu[1]
    prod_rand = (
        input_rand[2] * mu[0] + row1_rand[2] * mu[1] + hr[2] * mu[num_inputs]
    ) * nu[num_inputs - 1]
    rand = fnp.stack([a_rand, b_rand, prod_rand])

    return HpZkCore(
        instance, a_open, b_open, rand, fnp.stack(low), fnp.stack(high), hiding_comms
    )
...
def prove_zk_core(
    ...
    zero placeholder. `hp_rand` (the general prover) lifts the hiding values +
    randomizers
...
def prove_zk(
    ...
    a_vecs: list[list[int]],
    b_vecs: list[list[int]],
    ...
    Instance,
    tuple[frx.Array, frx.Array, frx.Array],
    list[np.ndarray],
    list[np.ndarray],
    Instance,
]:
...
def serialize_witness_zk(
    cv: Curve, witness: tuple[frx.Array, frx.Array, frx.Array]
) -> bytes:
    """`InputWitness` CanonicalSerialize (zk): `a_vec`, `b_vec`, then `Some`
    randomness (`rand_1, rand_2, rand_3`)."""
    a_vec, b_vec, rands = witness
    out = (
        curve.serialize_fr_vec(cv, a_vec) + curve.serialize_fr_vec(cv, b_vec) + b"\x01"
    )
...
def serialize_proof_zk(
    cv: Curve, low: list[np.ndarray], high: list[np.ndarray], hiding_comms: Instance
) -> bytes:
