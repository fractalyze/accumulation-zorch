...
// Pasta (Pallas) G1, standard form. The `PALLAS_G1_AFFINE` bases tag is what
// routes the MSM thunk to `RunPallasG1Msm` (`CurveOfBasesType` in
// msm_thunk.cc). Pasta is pairing-free, so G1 only. Coordinate/scalar byte
// sizes match BN254 (32-byte LE limbs), so `SF_BYTES`/`G1_BYTES` are reused.
pub const PALLAS_SF: sys::PJRT_Buffer_Type = sys::PJRT_Buffer_Type_PJRT_Buffer_Type_PALLAS_SF;
pub const PALLAS_G1_AFFINE: sys::PJRT_Buffer_Type = sys::PJRT_Buffer_Type_PJRT_Buffer_Type_PALLAS_G1_AFFINE;

// Pasta (Vesta) G1, standard form — the other half of the cycle. Same layout as
// Pallas; the `VESTA_G1_AFFINE` bases tag routes the MSM thunk to
// `RunVestaG1Msm` (`CurveOfBasesType` in msm_thunk.cc).
pub const VESTA_SF: sys::PJRT_Buffer_Type = sys::PJRT_Buffer_Type_PJRT_Buffer_Type_VESTA_SF;
pub const VESTA_G1_AFFINE: sys::PJRT_Buffer_Type = sys::PJRT_Buffer_Type_PJRT_Buffer_Type_VESTA_G1_AFFINE;
...
impl Session {
    ...
    /// Single Pallas G1 `lax.msm` against a pre-compiled executable: scalars
    /// (`PALLAS_SF`, `[n]`), Pallas G1 points (`PALLAS_G1_AFFINE`, `[n]`) -> one
    /// G1 point (64 bytes). The bases' `PALLAS_G1_AFFINE` tag is what routes the
    /// thunk to `RunPallasG1Msm`.
    pub unsafe fn run_msm_pallas(&self, exe: &Executable, scalars: &[u8], points: &[u8]) -> Vec<u8> {
        let ns = (scalars.len() / SF_BYTES) as i64;
        let np = (points.len() / G1_BYTES) as i64;
        let mut r = self.run(
            exe,
            &[(scalars, vec![ns], PALLAS_SF), (points, vec![np], PALLAS_G1_AFFINE)],
            1,
        );
        r.pop().unwrap()
    }

    /// Single Vesta G1 `lax.msm` against a pre-compiled executable: scalars
    /// (`VESTA_SF`, `[n]`), Vesta G1 points (`VESTA_G1_AFFINE`, `[n]`) -> one G1
    /// point (64 bytes). The bases' `VESTA_G1_AFFINE` tag routes the thunk to
    /// `RunVestaG1Msm`.
    pub unsafe fn run_msm_vesta(&self, exe: &Executable, scalars: &[u8], points: &[u8]) -> Vec<u8> {
        let ns = (scalars.len() / SF_BYTES) as i64;
        let np = (points.len() / G1_BYTES) as i64;
        let mut r = self.run(
            exe,
            &[(scalars, vec![ns], VESTA_SF), (points, vec![np], VESTA_G1_AFFINE)],
            1,
        );
        r.pop().unwrap()
    }
