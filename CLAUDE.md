...
- **Byte-identical to arkworks.** Every prover change is gated `fused GPU core ≡
  ...
  frx CPU port + the fused GPU core byte-match them. No behavior change ships without
  its byte-match.
- **Reuse the gadget, never re-implement it.** The in-circuit verifier gadget is
  ...
  lives only in the frx port (`python/accumulation_zorch/`). The repo re-derives
  neither.
