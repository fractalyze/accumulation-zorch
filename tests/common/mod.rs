//! The `PastaCurve`-generic face of the fixture codec, plus the one piece of
//! fixture parsing that is genuinely test-only.
//!
//! The codec itself lives in [`fixture_json`] — one definition, shared with the
//! `examples/` dumpers that write these fixtures. The curve-generic entry points
//! are re-exported as-is; the wrappers below exist only to bridge the two
//! generics: the GPU tests are generic over the `gpu`-gated `PastaCurve`, the
//! crate over `SWModelParameters`, and `<C as PastaCurve>::Params` is the
//! bridge. They add no encoding of their own.
//!
//! [`zk_inputs_from_seed`] is the test-only part and stays here: it is about the
//! fused core's *input shape*, not the JSON — it builds the general zk core's
//! runtime inputs from a fixture seed entry (the same per-seed
//! witness/randomness `export/export_prove.py build_core` reads), so one lowered
//! `prove_zk_general.mlirbc` is exercised per seed.
...
/// Hex codec — curve-generic already, so used straight from the crate.
/// (`allow` for the same reason as the module's `dead_code`: it covers unused
/// functions, but each test crate leaves some of these re-exports unused too.)
#[allow(unused_imports)]
pub use fixture_json::{from_hex, hex as to_hex};

/// A canonical-LE scalar field element from its fixture hex (the `fe_hex` form).
pub fn fr_from_hex<C: PastaCurve>(s: &str) -> Fr<C> {
    fixture_json::fe_from_hex(s)
}
...
///
/// Decodes via [`PointJson::to_affine`] (= `wire::g1_from_bytes`, what the GPU
/// MSM boundary uses), so the identity guard is not re-derived here.
pub fn point_from_json<C: PastaCurve>(v: &Value) -> Affine<C>
where
    Fq<C>: PrimeField,
{
    PointJson {
        x_le_hex: v["x_le_hex"].as_str().unwrap().to_string(),
        y_le_hex: v["y_le_hex"].as_str().unwrap().to_string(),
    }
    .to_affine::<C::Params>()
}

/// A fixture hex array as scalars. Walking the JSON array is the test-only part;
/// each element decodes through [`fr_from_hex`].
